      ...
      <div className="overflow-x-auto mt-4 pb-2">
        <svg width={width} height={height} role="img" aria-label={title}>
          {[0, 0.25, 0.5, 0.75, 1].map(tick => {
            ...
            const lineY = y(value);
            return <g key={tick}><line x1={padding.left} y1={lineY} x2={width-padding.right} y2={lineY} stroke="var(--chart-grid)"/><text x={padding.left-8} y={lineY+4} textAnchor="end" fontSize="10" fill="var(--chart-label)">{formatter(value)}</text></g>;
          })}
          <polyline fill="none" stroke={stroke} strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" points={points}/>
          {data.map((item,index) => {
            const value=Number(item[valueKey])||0;
            return <g key={item.month as string}><circle cx={x(index)} cy={y(value)} r="4" fill="var(--chart-card)" stroke={stroke} strokeWidth="3"><title>{item.month}: {formatter(value)}</title></circle>{(index%labelStep===0||index===data.length-1)&&<text x={x(index)} y={height-15} textAnchor="middle" fontSize="10" fill="var(--chart-label)">{item.month}</text>}</g>;
          })}
        </svg>
      </div>}
  </section>;
...
export const ReportsDashboard: React.FC<Props> = ({ orders, stats }) => {
  ...
  const canSendReports = !isGuest && ['admin', 'operations', 'analyst'].includes(role);
  ...
  const [senderEmail, setSenderEmail] = useState(() => localStorage.getItem('marketops-report-sender-email') || '');
  const [senderName, setSenderName] = useState(() => localStorage.getItem('marketops-report-sender-name') || 'MarketOps');
  ...
  const [emailStatus, setEmailStatus] = useState<{ configured: boolean; allowed_senders?: string[]; demo_redirect_configured: boolean; mode: string } | null>(null);
  useEffect(() => { localStorage.setItem('marketops-report-to', to); }, [to]);
  useEffect(() => { localStorage.setItem('marketops-report-cc', cc); }, [cc]);
  useEffect(() => { localStorage.setItem('marketops-report-sender-email', senderEmail); }, [senderEmail]);
  useEffect(() => { localStorage.setItem('marketops-report-sender-name', senderName); }, [senderName]);
  useEffect(() => { localStorage.setItem('marketops-report-language', reportLanguage); }, [reportLanguage]);
  useEffect(() => {
    if (!canSendReports) return;
    let active = true;
    apiFetch('/api/email/status')
      ...
      .then(status => { if (active) { setEmailStatus(status); setSenderEmail(current => current || status.allowed_senders?.[0] || ''); } })
      .catch(() => { if (active) setEmailStatus(null); });
    return () => { active = false; };
  }, [canSendReports]);
  ...
  const send=async(e:React.FormEvent)=>{e.preventDefault();setSending(true);setFeedback(null);try{const r=await apiFetch('/api/reports/email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to,cc,subject,message,language:reportLanguage,sender_email:senderEmail,sender_name:senderName})});const j=await r.json();if(!r.ok)throw new Error(j.error||(pt?'Não foi possível enviar.':'Unable to send.'));setFeedback({ok:true,text:pt?`Relatório enviado por ${senderEmail} para ${to}.`:`Report sent by ${senderEmail} to ${to}.`});}catch(err:any){setFeedback({ok:false,text:err.message});}finally{setSending(false);}};
  ...
  return <div className="space-y-6">
    {canSendReports && <section className="rounded-2xl border border-blue-200 bg-blue-50/40 p-4"><h2 className="font-bold text-slate-900">{pt?'Identificação do remetente':'Sender identity'}</h2><p className="mt-1 text-xs text-slate-500">{pt?'Essa identificação será usada no envio do relatório. O endereço precisa ser a conta SMTP ou um alias autorizado pelo Google.':'This identity will be used for the report. The address must be the SMTP account or a Google-authorized alias.'}</p><div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2"><label className="text-xs font-semibold">{pt?'Nome do remetente':'Sender name'}<input required type="text" placeholder="MarketOps — Isabela" value={senderName} onChange={e=>setSenderName(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-2.5 text-slate-900"/></label><label className="text-xs font-semibold">{pt?'E-mail do remetente':'Sender email'}<input required type="email" list="marketops-allowed-senders" value={senderEmail} onChange={e=>setSenderEmail(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-2.5 text-slate-900"/><datalist id="marketops-allowed-senders">{emailStatus?.allowed_senders?.map(address=><option key={address} value={address}/>)}</datalist></label></div></section>}
    ...
    {canSendReports && <section className="bg-white border border-blue-200 rounded-2xl p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold flex gap-2"><Mail className="w-5 h-5 text-blue-600"/>{pt?'Enviar relatório completo por e-mail':'Send Full Report by Email'}</h2><p className="text-xs text-slate-500 mt-1">{pt?'Os destinatários ficam salvos neste navegador. Use ponto e vírgula para vários endereços.':'Recipients are saved in this browser. Use semicolons for multiple addresses.'}</p></div>{emailStatus && <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold ${emailStatus.configured?'border-emerald-200 bg-emerald-100 text-emerald-800':'border-amber-200 bg-amber-100 text-amber-900'}`}>{emailStatus.configured?<CheckCircle2 className="w-3.5 h-3.5"/>:<AlertCircle className="w-3.5 h-3.5"/>}{emailStatus.configured?(pt?'SMTP conectado neste ambiente':'SMTP connected in this environment'):(pt?'SMTP não configurado neste ambiente':'SMTP not configured in this environment')}</span>}</div>{emailStatus?.configured===false&&<div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/80 p-3 text-xs text-amber-950">{pt?'Para o envio funcionar no site publicado, configure SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASSWORD e SMTP_FROM nas variáveis Environment do Render.':'To send from the deployed site, configure SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASSWORD and SMTP_FROM in the Render Environment variables.'}</div>}<form onSubmit={send} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5"><label className="text-xs font-semibold">{pt?'Para':'To'}<input required type="text" placeholder="email1@company.com; email2@company.com" value={to} onChange={e=>setTo(e.target.value)} className="mt-1 w-full p-2.5 rounded-xl border border-slate-300 bg-white text-slate-900"/></label><label className="text-xs font-semibold">Cc ({pt?'opcional':'optional'})<input placeholder="email3@company.com" value={cc} onChange={e=>setCc(e.target.value)} className="mt-1 w-full p-2.5 rounded-xl border border-slate-300 bg-white text-slate-900"/></label><label className="md:col-span-2 text-xs font-semibold">{pt?'Assunto':'Subject'}<input required value={subject} onChange={e=>setSubject(e.target.value)} className="mt-1 w-full p-2.5 rounded-xl border border-slate-300 bg-white text-slate-900"/></label><label className="md:col-span-2 text-xs font-semibold">{pt?'Mensagem':'Message'}<textarea rows={4} value={message} onChange={e=>setMessage(e.target.value)} className="mt-1 w-full p-2.5 rounded-xl border border-slate-300 bg-white text-slate-900"/></label><div className="md:col-span-2 flex justify-between items-center gap-3">{feedback?<span className={feedback.ok?'text-xs text-emerald-700':'text-xs text-red-700'}>{feedback.text}</span>:<span/>}<button disabled={sending||emailStatus?.configured===false} className="flex gap-2 items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed">{sending?<Loader2 className="w-4 h-4 animate-spin"/>:<Send className="w-4 h-4"/>}{sending?(pt?'Enviando...':'Sending...'):(pt?'Enviar relatório':'Send Report')}</button></div></form></section>}
  </div>;
};
